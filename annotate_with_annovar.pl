...
# $command = "table_annovar.pl $destination/annovar.vcf $maf_database -buildver hg18 -out $destination/annovar -remove -protocol knownGene,ljb26_all,refGene,ensGene -operation g,f,g,g -nastring . -vcfinput";
print STDERR "$command\n" if $flag_debug;
system($command);
