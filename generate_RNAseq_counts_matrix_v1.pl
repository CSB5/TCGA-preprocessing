...
my $help_message = "
This script generates a matrix containing the RNAseq counts from TCGA data.

Usage:
	generate_RNAseq_counts_matrix.pl [OPTIONS]
...
unless ( -s "$expTempDir/FILE_SAMPLE_MAP_NORMAL.txt" ) {
	$command =
"cut -f2 $config{'general.dataDir'}/$config{'exp.sdrf'} | sort | uniq | grep -f $config{'general.dataDir'}/$config{'exp.normalList'} - | grep -f - $expTempDir/FILE_SAMPLE_MAP.txt | sort -k2,2 > $expTempDir/FILE_SAMPLE_MAP_NORMAL.txt";
	system($command);
	print TRACE "[EXP] $command\n" if ($flag_debug);
}
...
$command =
"$config{'general.scriptsDir'}/DESeq_pooled_normalization.R $expTempDir/RNA_SEQ_COUNT_MATRIX.dat $noNormals";
system($command);
print TRACE "[EXP] $command\n" if ($flag_debug);
